...
void test_small_rw_operation(){
  ...
  // char* maxBlock = malloc((FS->superblock.numDataBlocks-1)*BLOCK_SIZE);
  // char* maxBlock2 = malloc(FS->superblock.numDataBlocks*BLOCK_SIZE);
  // for(int i = 0; i < FS->superblock.numDataBlocks * BLOCK_SIZE; i++) {
  //   maxBlock[i] = 3;
  // }
  // for(int i = 0; i < (FS->superblock.numDataBlocks-1)*BLOCK_SIZE; i++) {
  //   maxBlock2[i] = 4;
  // }
  // assert(fs_create("test-fs-3") == 0);
  // assert(fs_open("test-fs-3") == 2);
  // int i = fs_write(2, maxBlock, (FS->superblock.numDataBlocks-1)*BLOCK_SIZE);
  // printf("i is %d\n", i);

  // assert(i == (FS->superblock.numDataBlocks-1)*BLOCK_SIZE);
...
void test_open_and_create(){
  ...
  for(int i = 0; i < FS_FILE_MAX_COUNT + 3; i++) {
    sprintf(buffer, "tmc%d", i);
    if(i >= FS_FILE_MAX_COUNT) {
      assert(fs_create("test-out-bounds") == -1);
      break;
    }
    int z = fs_create(buffer);
    if(z != 0){
    	printf("Bad file number: %d\n", i);
    }
    assert(z == 0);
  }
